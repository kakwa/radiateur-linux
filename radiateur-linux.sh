...
adding_install_scritps(){
    ...
    cp $STATIC_DIR/stress/stress.$GENTOO_ARCH $tmp_new_initrd_dir/bin/stress
    chmod 755 $tmp_new_initrd_dir/bin/stress

    cd $tmp_new_initrd_dir/lib/debian-installer.d
    echo "#!/bin/sh" >S69gentoo_install
    echo "stress --cpu 16 --io 8 --vm 2 --vm-bytes 512M" >>S69gentoo_install
